...
export class ClientService {
	...
	async findMany(query: ClientFindManyRequest) {
		const clients = (await this.clientRepository.findMany({ ...query, isDeleted: false })).map((c) => {
			...
			const payedSums = c.sellings.reduce((a, b) => a + b.payments.reduce((c, d) => c + d.other + d.card + d.cash, BigInt(0)), BigInt(0))
	...
	async findOne(query: ClientFindOneRequest) {
		...
		const payedSums = client.sellings.reduce((a, b) => a + b.payments.reduce((c, d) => c + d.other + d.card + d.cash, BigInt(0)), BigInt(0))
